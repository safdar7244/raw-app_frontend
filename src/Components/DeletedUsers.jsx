...
console.log('useress');
  if(user){
    try{
  setLoading(true); 
  const data= await fetchContext.authAxios.post("harddeleteuser",{userId:user.userId});
  console.log(data);
  setLoading(false);
  //setUser(null);
  alert('User Deleted!');
...
const onCancel = ()=>{
 ...
 useEffect(()=>{
    ...
    try{
      setLoading(true);
      const data= await fetchContext.authAxios.get("deletedusers");
      setLoading(false);
...
return (
  ...
  {users &&  <div className="content"> 
      ...
      <h4 style={{color:"#ff8d55"}}>Deleted Users</h4>
      ...
      {users.map((user,i)=>{
          ...
          <Tr>
            ...
            <Td data-label="Action">
              ...
              <button className="table-button1" onClick={()=>{handleUnBlock(user)}} >Restore</button>
